...
const updateTaskController = async (req, res, next) => {
  try {
    ...
    await updateTaskService(taskid, task, status); // Não enviar o userId
    return res.status(200).json({ message: `${taskid} updated` });
  } catch (error) {
    console.log(error.message);
    next(error);
  }
...
const removeTaskController = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ message: `${taskid} removed` });
  } catch (error) {
    console.log(error.message);
    next(error);
  }
